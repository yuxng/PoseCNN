...
LOG="experiments/logs/rgbd_scene_vgg16_test_single_frame.txt.`date +'%Y-%m-%d_%H-%M-%S'`"
exec &> >(tee -a "$LOG")
...
time ./tools/test_net.py --gpu 0 \
  --network vgg16_convs \
  --model data/fcn_models/rgbd_scene/rgbd/vgg16_fcn_rgbd_single_frame_rgbd_scene_iter_40000.ckpt \
  --imdb rgbd_scene_val \
