...
cdef extern from "synthesizer.hpp":
    cdef cppclass Synthesizer:
        ...
        void render(int, int, float, float, float, float, float, float, unsigned char*, float*, float*, float*, float*)
...
cdef class PySynthesizer:
    ...
    def render(self, np.ndarray[np.uint8_t, ndim=3] color, np.ndarray[np.float32_t, ndim=2] depth, np.ndarray[np.float32_t, ndim=3] vertmap, \
               ...
               np.float32_t fx, np.float32_t fy, np.float32_t px, np.float32_t py, np.float32_t znear, np.float32_t zfar):

        cdef unsigned char* color_buff = <unsigned char*> color.data
        cdef int height = color.shape[0]
        cdef int width = color.shape[1]

        return self.synthesizer.render(width, height, fx, fy, px, py, znear, zfar, color_buff, &depth[0, 0], &vertmap[0, 0, 0], &class_indexes[0], &poses[0, 0])
